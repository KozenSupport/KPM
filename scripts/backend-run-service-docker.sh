...
docker run -d --rm \
  --name "$MODULE-dev" \
  -p "$PORT:$PORT" \
  -e KPM_NACOS_ADDR="host.docker.internal:8848" \
  ...
  -e JAVA_TOOL_OPTIONS="${KPM_JAVA_TOOL_OPTIONS:--Xms64m -Xmx256m -XX:+UseG1GC}" \
  -e KPM_DB_HOST="${KPM_DB_HOST:-host.docker.internal}" \
  ...
  -e KPM_IAM_URI="${KPM_IAM_URI:-http://host.docker.internal:8101}" \
  -e KPM_RESOURCE_URI="${KPM_RESOURCE_URI:-http://host.docker.internal:8102}" \
  -e KPM_PROJECT_URI="${KPM_PROJECT_URI:-http://host.docker.internal:8103}" \
  -e KPM_CUSTOMER_URI="${KPM_CUSTOMER_URI:-http://host.docker.internal:8104}" \
  -e KPM_TASK_URI="${KPM_TASK_URI:-http://host.docker.internal:8105}" \
  -e KPM_ORDER_URI="${KPM_ORDER_URI:-http://host.docker.internal:8106}" \
  -e KPM_FILE_URI="${KPM_FILE_URI:-http://host.docker.internal:8107}" \
  -e KPM_ANALYTICS_URI="${KPM_ANALYTICS_URI:-http://host.docker.internal:8108}" \
  -e KPM_NOTIFICATION_URI="${KPM_NOTIFICATION_URI:-http://host.docker.internal:8110}" \
  ...
  sh -lc "if [ -f '$MODULE/target/$MODULE-0.1.0-SNAPSHOT.jar' ]; then java -jar '$MODULE/target/$MODULE-0.1.0-SNAPSHOT.jar'; else mvn -q -pl '$MODULE' -am -DskipTests install && mvn -pl '$MODULE' spring-boot:run; fi"
