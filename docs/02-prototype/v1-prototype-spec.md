...
- 客户状态
- 支持查询、新增、编辑、删除
- 客户录入 / 维护权限可通过角色授权或用户直授控制

#### 客户状态配置

- 维护项目客户状态枚举值
- 不需要模板
- 仅有权限的用户可查询、新增、编辑、启用 / 停用、删除状态
...
- 支持订单新增、编辑、删除、查询
- 字段包括：下单日期、下单客户、项目信息、数量、具体规格、期望发货日期、计划发货日期、软件版本号、币种、单价、订单金额、创建人
- 订单金额由数量 × 单价计算
- 订单修改时记录修改人、时间、内容和原因
- 订单 CRUD 纳入权限配置
...
- 二级菜单：订单统计 / 资源分配情况 / 技术支持情况
- 订单统计支持按年、季度、月份和项目筛选
- 资源分配情况以可拖拽地图展示客户、销售、技术支持和已售项目
- 技术支持情况以树状图展示客户、技术支持、当前任务统计和卡点任务
...
- 能在资源管理中查询、新增、编辑用户
- 能在资源管理中停用 / 启用用户
- 能在资源管理中对部门、角色、权限、客户执行查询 / 新增 / 编辑 / 删除
- 能在资源管理中对客户状态执行查询 / 新增 / 编辑 / 启停 / 删除
- 能在资源管理中配置任务状态与流转关系
- 能在任务管理页创建任务
- 能从阶段详情创建任务
- 能按条件筛选任务
